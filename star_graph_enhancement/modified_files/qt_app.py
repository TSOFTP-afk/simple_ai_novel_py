...
from PyQt6.QtGui import QAction, QBrush, QColor, QCloseEvent, QFont, QImage, QKeySequence, QPainter, QPixmap, QShortcut, QTextCharFormat, QTextCursor, QStandardItem, QStandardItemModel
...
class NovelQtMainWindow(QMainWindow):
    ...
    def _build_toolbar(self) -> None:
        groups = {
            ...
            "AI": [
                ...
                ("多智能体生成", self._start_multi_agent_generation),
                ("续写", lambda: self._start_generation("continue")),
                ...
                ("多智能体审查", self._review_current_scope),
            ...
            "视图": [
                ...
                ("审查抽屉", lambda: self._open_drawer("review")),
    ...
    def _build_workspace_panel(self) -> QFrame:
        ...
        for label, callback in [
            ...
            ("📋 审查", self._review_current_scope),
            ("🤖 生成", self._start_multi_agent_generation),
        ]:
    ...
    def _build_drawer_tabs(self) -> QTabWidget:
        ...
        add_tab("outline", self._build_outline_tab(), "大纲")
        ...
        add_tab("review", self._build_review_tab(), "审查")
    ...
    def _build_outline_tab(self) -> QWidget:
        ...
        label = QLabel("章节大纲")
        label.setObjectName("SectionTitle")
        sync_btn = QPushButton("AI 同步大纲")
        sync_btn.clicked.connect(self._refresh_summary)
    ...
    def _build_star_graph_tab(self) -> QWidget:
        ...
        self.star_graph_style_manager = StyleManager(self.db, self.state.selected_book_id)
        self.star_graph_style_manager.initialize_defaults()
        self.star_graph.set_style_manager(self.star_graph_style_manager)
        self.star_graph.createCharacterRequested.connect(self._create_character_from_graph)
        self.star_graph.characterMoved.connect(self._move_character_in_graph)
        self.star_graph.relationshipRequested.connect(self._create_relationship_from_graph)
        self.star_graph.editCharacterRequested.connect(self._edit_character_from_graph)
        self.star_graph.deleteCharacterRequested.connect(self._delete_character_from_graph)
        self.star_graph.editRelationshipRequested.connect(self._edit_relationship_from_graph)
        self.star_graph.deleteRelationshipRequested.connect(self._delete_relationship_from_graph)
        self.star_graph.changeCharacterStyleRequested.connect(self._change_character_style_from_graph)
        self.star_graph.changeRelationshipTypeRequested.connect(self._change_relationship_type_from_graph)
        self.star_graph.chapterSelected.connect(self._jump_to_chapter_from_graph)
    ...
    def _build_skills_tab(self) -> QWidget:
        ...
        layout.addWidget(QLabel("已启用 Skills"))
    ...
    def _build_review_tab(self) -> QWidget:
        ...
        run_btn = QPushButton("多智能体审查")
        run_btn.clicked.connect(self._review_current_scope)
    ...
    def _build_task_tab(self) -> QWidget:
        ...
        layout.addWidget(self.task_summary_panel)
    ...
    def _setup_shortcuts(self) -> None:
        ...
        QShortcut(QKeySequence("Ctrl+Shift+R"), self, activated=self._review_current_scope)
    ...
    def _refresh_overview(self) -> None:
        ...
        self.overview_meta.setText(f"卷 {len(volumes)} · 章节 {len(chapters)} · Skills {len(self.book_skills) + len(self.bound_skills)}")
    ...
    def _refresh_star_graph(self) -> None:
        ...
        if not hasattr(self, "star_graph_style_manager"):
            ...
            self.star_graph_style_manager.initialize_defaults()
            self.star_graph.set_style_manager(self.star_graph_style_manager)
        relationships = [dict(item) for item in self.database.list_relationships(self.state.selected_book_id)]
        chapters = [dict(item) for item in self.database.list_chapters(self.state.selected_book_id)]
        events = [dict(item) for item in self.database.list_chapter_events(self.state.selected_book_id)]
        self.star_graph.load_book(
            [dict(item) for item in self.characters],
            relationships,
            chapters,
            events,
    ...
    def _change_character_style_from_graph(self, character_id: int) -> None:
        ...
        if dialog.exec() == dialog.Accepted and dialog.selected_style_id is not None:
            ...
            if style:
                self.db.set_character_node_style(character_id, style.id)
    ...
    def _change_relationship_type_from_graph(self, relationship_id: int) -> None:
        ...
        if relationship and relationship.get("relationship_type_id"):
            current_type_id = int(relationship["relationship_type_id"])
        ...
        if dialog.exec() == dialog.Accepted and dialog.selected_type_id is not None:
            self.db.update_relationship_type_info(relationship_id, dialog.selected_type_id, dialog.description)
            self._refresh_star_graph()
    ...
    def _refresh_skills(self) -> None:
        self.enabled_skills_list.clear()
        for item in self._get_bound_skill_payload():
            list_item = QListWidgetItem(f"{item.get('name', '')} · {item.get('category', '')}")
    ...
    def _refresh_review_tab(self) -> None:
        ...
        if self.review_runs_list.count():
            ...
            self.review_detail.setPlainText("暂无审查记录。")
    ...
    def _render_review_run_detail(self, run_id: int) -> None:
        run = self.database.get_review_run(run_id)
        if not run:
            self.review_detail.setPlainText("审查记录不存在。")
            return
        findings = self.database.list_review_findings(run_id)
    ...
    def _copy_selected_review_truth_file(self) -> None:
        ...
        if not truth:
            info(self, "Truth File", "当前审查记录没有 Truth File。")
            return
    ...
    def _get_bound_skill_payload(self) -> list[dict[str, Any]]:
        ...
        seen: set[int] = set()
        for item in [*self.book_skills, *self.bound_skills]:
    ...
    def _apply_library_collapsed_state(self) -> None:
        ...
        if self.library_collapsed:
            if can_resize:
                ...
                self.main_splitter.setSizes([88, max(800, sizes[1])])
            self.library_title.setText("书库")
    ...
    def _open_ai_chat(self) -> None:
        ...
        if self.chat_dialog is None:
            self.chat_dialog = ChatDialog(self, ai_service, self._build_chat_context_prompt)
    ...
    def _build_chat_context_prompt(self) -> str:
        lines = [
    ...
    def _start_multi_agent_generation(self) -> None:
        ...
        if self._has_active_write_job(target_chapter_id):
            ...
            return
        if self._find_active_background_job("多智能体生成"):
            info(self, "任务运行中", "多智能体生成已经在运行。")
            return
        ai_service = self._require_ai_service("review", "多智能体生成")
        if ai_service is None:
            return
        outline = str(chapter["outline"] or self.chapter_outline_editor.toPlainText()).strip()
        current_content = str(chapter["content"] or self.content_editor.toPlainText()).strip()
        if not outline:
            info(self, "提示", "请先为当前章节写入大纲，再启动多智能体生成。")
            return
        snapshot_id = self.database.create_snapshot(
            target_chapter_id,
            f"多智能体生成前 · {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            outline,
            current_content,
        )
        target_payload = {
            ...
            "truth_file": self._build_review_truth_file(dict(chapter), [dict(chapter)]),
        ...
        self._open_drawer("tasks")
        self._append_task_log(f"多智能体生成启动：{target_payload['title']}，参考前 {len(target_payload['previous_chapters'])} 个章节。")

        def work(log: Callable[[str], None], cancel_event: Event | None) -> dict[str, Any]:
            log("多智能体生成：大纲拆解、正文写作、审查修订开始。")
            review = ai_service.multi_agent_generate_from_outline(
                ...
                previous_chapters=list(target_payload["previous_chapters"]),
            )
            if cancel_event and cancel_event.is_set():
                log("多智能体生成已停止，结果不会覆盖正文。")
                return {
    ...
    def _start_generation(self, mode: str) -> None:
        ...
        if mode in {"continue", "polish"} and not current_content:
            info(self, "提示", "当前正文为空，请先运行多智能体生成或手动写入正文。")
            return
    ...
    def _refresh_summary(self) -> None:
        ...
        if self._has_active_write_job(self.state.selected_chapter_id, targets={"outline"}):
            ...
            return
        ai_service = self._require_ai_service("outline", "AI 同步大纲")
        if ai_service is None:
            return
    ...
    def _review_current_scope(self) -> None:
        if not self.state.selected_book_id:
            info(self, "多智能体审查", "请先选择一本书、一个卷或一个章节。")
            return
        ai_service = self._require_ai_service("review", "多智能体审查")
        if ai_service is None:
            return
        if self._find_active_background_job("多智能体审查"):
            info(self, "任务运行中", "多智能体审查已经在运行。")
            return
        if self.state.editor_dirty and not self._save_current():
            return
        targets = self._collect_review_targets()
        if not targets:
            info(self, "多智能体审查", "当前范围没有可审查的章节。")
            return
        ...
        self._open_drawer("tasks")
        self._append_task_log(f"多智能体审查准备：{len(targets)} 章")
        ...
        for chapter in targets:
            ...
            snapshot_id = self.database.create_snapshot(
                chapter_id,
                f"多智能体审查前 · {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                base_outline,
                base_content,
            )
            prepared_targets.append(
                {
                    ...
                    "snapshot_id": snapshot_id,
                    "truth_file": self._build_review_truth_file(chapter, targets),
        ...
        def work(log: Callable[[str], None], cancel_event: Event | None) -> dict[str, Any]:
            ...
            for index, target in enumerate(prepared_targets, start=1):
                if cancel_event and cancel_event.is_set():
                    ...
                    log("多智能体审查已停止，后续章节不会写入。")
                    break
                log(f"多智能体审查 {index}/{len(prepared_targets)}：{target['title']}")
                review = ai_service.multi_agent_review(
                    book_title=book_title,
                ...
                if index > 1:
                    ...
                    cross_result = ai_service.cross_chapter_check(
                ...
                log(f"多智能体审查进度：已完成 {len(results)}/{len(prepared_targets)}")
            return {
    ...
    def _build_review_truth_file(self, chapter: dict[str, Any], scope_chapters: list[dict[str, Any]]) -> str:
        ...
        lines.append("审查机制来源：借鉴 InkOS 的多智能体审查范式，仅做机制级学习，不复用其源码。")
    ...
    def _apply_multi_agent_review_result(self, payload: Any) -> None:
        ...
        if not isinstance(results, list):
            return
        source_label = str(payload.get("source_label", "多智能体审查") or "多智能体审查")
    ...
    def _update_task_activity_state(self) -> None:
        ...
        self._set_task_summary_expanded(active > 0, animated=True)
    ...
    def _finalize_stale_cancelled_jobs(self) -> None:
        ...
        for job_id, job in stale_jobs:
            ...
            if isinstance(thread, QThread) and thread.isRunning():
                ...
                thread.finished.connect(lambda jid=job_id: self.detached_cancelled_jobs.pop(jid, None))
                thread.requestInterruption()
                thread.quit()
                detached += 1
            self.ai_jobs.pop(job_id, None)
    ...
    def _open_book_settings(self, book_id: int | None = None) -> None:
        ...
        if not book_id:
            ...
            return
        dialog = QDialog(self)
        dialog.setWindowTitle("书籍设置 / Skills 管理")
        ...
        def load_enabled() -> None:
            state["enabled"] = [dict(item) for item in self.database.list_bound_skills_for_book(book_id)]
            enabled_list.clear()
        ...
        def bind_selected() -> None:
            ...
            self.database.bind_skill_to_book(book_id, int(skill["id"]))
            load_enabled()
            self._refresh_drawer()
        ...
        def unbind_selected() -> None:
            ...
            if not current:
                return
            skill = current.data(Qt.ItemDataRole.UserRole)
            self.database.unbind_skill_from_book(book_id, int(skill["id"]))
            load_enabled()
            self._refresh_drawer()
    ...
    def resizeEvent(self, event) -> None:  # noqa: N802
        ...
        width = self.width()
        if width < 900 and not self.library_collapsed:
            self.main_splitter.setSizes([88, max(700, self.main_splitter.sizes()[1])])
    ...
    def closeEvent(self, event: QCloseEvent) -> None:  # noqa: N802
        if self.state.task_running or self.detached_cancelled_jobs:
            if not confirm(self, "任务运行中", "AI 任务仍在运行或退出中，确定退出吗？", danger=True):
                event.ignore()
                return
            self._cancel_task()
            for job in list(self.detached_cancelled_jobs.values()):
                thread = job.get("thread")
                if isinstance(thread, QThread):
                    thread.requestInterruption()
                    thread.quit()
